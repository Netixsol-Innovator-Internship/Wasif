document.addEventListener("DOMContentLoaded", () => {
  ...
  notifications.forEach((element) => {
    ...
    li.classList = `${
      element.unread ? "bg-gray-100" : "bg-white"
    } text-black text-xs py-3 px-2 md:px-1 rounded-lg w-full hover:bg-gray-200 cursor-pointer notificationItem`;
        ...
        <div class="flex gap-x-2 items-start">
          ...
          <div class="flex-1">
            <p class="text-sm leading-5">
              ...
              <span class="text-gray-600"> ${element.action} </span>
              ${
                element.target
                  ? `<span class="font-semibold cursor-pointer ${
                      element.chess ? "text-blue-950" : "text-gray-600"
                    }"> ${element.target}</span>`
                  : ""
              }
            ...
            <div class="flex md:justify-between">
              <p class="text-gray-400 text-xs mt-1">${element.time}</p>
              ${
                element.picture
            ...
            ${
              element.message
                ? `<div id="msg" class="justify-between border border-gray-300 p-3 mt-2 text-gray-600 rounded-md text-sm hidden"><p>${
                    element.message
                  }</p>
                ${
                  element.picture ? `<img class="inline-block w-28 md:w-56 h-22 md:h-44" src=${element.picture}></img>` : ""
                }</div>`
                : ""
            }
          </div>
